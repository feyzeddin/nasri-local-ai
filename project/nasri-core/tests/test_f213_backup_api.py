...
def test_backup_run_and_history(monkeypatch):
    ...
    async def _fake_run(**_kwargs):
        return {
            "backup_id": "b1",
            "created_at": 1,
            "output_path": "C:/tmp/b1.bin",
            "encrypted": True,
            "size_bytes": 10,
            "remote_status": "skipped:no-target",
        }

    async def _fake_hist(**_kwargs):
        return [
            {
                "backup_id": "b1",
                "created_at": 1,
    ...
    monkeypatch.setattr(backup_api, "run_backup", _fake_run)
    monkeypatch.setattr(backup_api, "backup_history", _fake_hist)

    run_resp = client.post("/backup/run", headers={"X-Session-Token": op})
    assert run_resp.status_code == 200
    hist_resp = client.get("/backup/history", headers={"X-Session-Token": vw})
    assert hist_resp.status_code == 200
    assert hist_resp.json()["count"] == 1


def test_backup_run_forbidden_for_viewer(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    vw = _token(client, "viewer", "viewer")
    resp = client.post("/backup/run", headers={"X-Session-Token": vw})
