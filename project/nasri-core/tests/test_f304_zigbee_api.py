...
import app.api.zigbee as zigbee_api
...
class _Settings:
    def __init__(self, rbac_enabled: bool = True) -> None:
        ...
        self.codegen_output_root = ".nasri-codegen"
        self.zigbee_enabled = True
        self.zigbee2mqtt_api_url = "http://localhost:8080"
        self.zigbee2mqtt_api_key = ""
...
def _make_client(monkeypatch, settings: _Settings) -> TestClient:
    ...
    monkeypatch.setattr(zigbee_api, "get_settings", lambda: settings)
...
def test_zigbee_status_and_devices(monkeypatch):
    ...
    async def _devices():
        return [{"friendly_name": "lamp_1", "model": "M1"}]

    monkeypatch.setattr(zigbee_api, "bridge_status", _status)
    monkeypatch.setattr(zigbee_api, "list_devices", _devices)

    assert client.get("/zigbee/status", headers={"X-Session-Token": vw}).status_code == 200
    devices = client.get("/zigbee/devices", headers={"X-Session-Token": vw})
    assert devices.status_code == 200
    assert devices.json()["count"] == 1


def test_zigbee_permit_join_forbidden_for_viewer(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    vw = _token(client, "viewer", "viewer")
    resp = client.post(
        "/zigbee/permit-join",
