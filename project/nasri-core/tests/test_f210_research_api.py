...
from app.services.research import ResearchError, ResearchResultItem
...
def test_research_query_success(monkeypatch):
    ...
    monkeypatch.setattr(research_api, "run_research", _fake_run)
    resp = client.post(
        "/research/query",
        json={"query": "nasri nedir", "max_results": 3, "save_report": True},
...
def test_research_query_error(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    token = _token(client, "viewer", "viewer")

    async def _fake_run(**_kwargs):
        raise ResearchError("searxng down")
