...
import app.api.anomaly as anomaly_api
...
def test_anomaly_ingest_and_alert_list(monkeypatch):
    ...
    assert ingest.json()["anomaly"] is True

    alerts = client.get("/anomaly/alerts", headers={"X-Session-Token": vw})
    assert alerts.status_code == 200
    assert alerts.json()["count"] >= 1


def test_anomaly_ingest_requires_operator(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    vw = _token(client, "viewer", "viewer")
    resp = client.post(
        "/anomaly/ingest",
        json={"event_type": "network", "actor": "a1"},
        headers={"X-Session-Token": vw},
    )
    assert resp.status_code == 403


def test_anomaly_status_available(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    vw = _token(client, "viewer", "viewer")
    resp = client.get("/anomaly/status", headers={"X-Session-Token": vw})
    assert resp.status_code == 200
