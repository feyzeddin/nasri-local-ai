...
def test_planner_run_success(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    viewer = _token(client, "viewer", "viewer")
    resp = client.post(
        "/planner/run",
        json={"goal": "Kullanici profilini hatirla ve cevap taslagi hazırla", "profile_id": "feyza"},
    ...
    assert any(step["action"] == "finalize" for step in data["steps"])
