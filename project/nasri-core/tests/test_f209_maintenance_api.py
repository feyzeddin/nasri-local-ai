...
import app.api.maintenance as m_api
...
def test_maintenance_status_success(monkeypatch):
    ...
    monkeypatch.setattr(m_api, "get_maintenance_status", _fake_status)
    resp = client.get("/maintenance/status", headers={"X-Session-Token": token})
    assert resp.status_code == 200
    assert resp.json()["last_result"] == "ok"


def test_maintenance_run_requires_operator(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    token = _token(client, "viewer", "viewer")
    resp = client.post("/maintenance/run", headers={"X-Session-Token": token})
