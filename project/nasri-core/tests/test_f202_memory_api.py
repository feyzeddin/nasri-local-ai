...
def test_memory_store_and_recall(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    import app.api.memory as memory_api

    monkeypatch.setattr(memory_api, "store_memory", lambda *_args, **_kwargs: "mem-1")
    monkeypatch.setattr(
        memory_api,
        "recall_memory",
        lambda *_args, **_kwargs: [
            {"memory_id": "mem-1", "text": "feyza prefers local mode", "score": 0.92, "tags": ["pref"]}
        ],
    )
    ...
    s = client.post(
        "/memory/store",
        json={"profile_id": "feyza", "text": "feyza prefers local mode", "tags": ["pref"]},
        headers={"X-Session-Token": operator},
    )
    assert s.status_code == 200
    assert s.json()["memory_id"] == "mem-1"

    r = client.get(
        "/memory/recall",
        params={"profile_id": "feyza", "q": "mode"},
        headers={"X-Session-Token": viewer},
    )
    assert r.status_code == 200
    assert r.json()["items"][0]["memory_id"] == "mem-1"
