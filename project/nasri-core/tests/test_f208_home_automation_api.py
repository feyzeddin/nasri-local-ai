...
import app.api.home_automation as ha_api
...
from app.services.home_automation import HomeAutomationError
...
def test_home_automation_command_success(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    token = _token(client, "operator", "operator")

    async def _fake_run_command(**_kwargs):
        return "mqtt", "turn_on", "light"

    monkeypatch.setattr(ha_api, "run_command", _fake_run_command)
    resp = client.post(
        "/home-automation/command",
        json={"command_text": "salondaki ışığı aç", "mode": "auto"},
        headers={"X-Session-Token": token},
    )
    assert resp.status_code == 200
    assert resp.json()["mode_used"] == "mqtt"


def test_home_automation_requires_operator_or_admin(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
...
def test_home_automation_error(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    token = _token(client, "operator", "operator")

    async def _fake_run_command(**_kwargs):
        raise HomeAutomationError("işlenemedi")

    monkeypatch.setattr(ha_api, "run_command", _fake_run_command)
    resp = client.post(
        "/home-automation/command",
        json={"command_text": "test", "mode": "auto"},
        headers={"X-Session-Token": token},
    )
