...
import app.api.network as network_api
...
from app.services.external_ai import ExternalAIResult
from app.services.model_router import RouterAttempt, RouterResult
from app.services.network import DiscoveredDevice
from app.services.research import ResearchResultItem
...
def test_phase2_integration_happy_path(monkeypatch):
    ...
    monkeypatch.setattr(
        planner_api,
        "run_planner",
        lambda _goal, profile_id=None: (
            True,
            "analyze_goal -> finalize",
            [],
        ),
    )
    monkeypatch.setattr(
        model_router_api,
        "route_chat",
        lambda **_kwargs: RouterResult(
            reply="ok",
            used_tier="local",
            attempts=[RouterAttempt(tier="local", status="ok")],
        ),
    )
    monkeypatch.setattr(
        external_ai_api,
        "send_chat",
        lambda **_kwargs: ExternalAIResult(
            provider="groq",
            model="llama",
            reply="ok",
            input_tokens=1,
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (
            [
                ResearchResultItem(
                    title="Nasri",
                    url="https://wikipedia.org/wiki/Nasri",
    ...
    assert client.get("/backup/history", headers={"X-Session-Token": vw}).status_code == 200


def test_phase2_regression_role_guards(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    vw = _token(client, "viewer", "viewer")
    assert client.post("/driver/install", json={"device_id": "x"}, headers={"X-Session-Token": vw}).status_code == 403
    assert client.post("/backup/run", headers={"X-Session-Token": vw}).status_code == 403
    assert client.post("/maintenance/run", headers={"X-Session-Token": vw}).status_code == 403
