...
def _make_client(monkeypatch, settings: _Settings) -> TestClient:
    ...
    monkeypatch.setattr(network_api, "get_settings", lambda: settings)
...
def test_network_discover_success(monkeypatch):
    ...
    resp = client.post(
        "/network/discover",
        json={"target_cidr": "192.168.1.0/24", "include_mdns": True},
        headers={"X-Session-Token": token},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["device_count"] == 1
    assert body["devices"][0]["ip"] == "192.168.1.10"


def test_network_discover_requires_operator_or_admin(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
...
def test_network_discover_error(monkeypatch):
    client = _make_client(monkeypatch, _Settings(rbac_enabled=True))
    token = _token(client, "operator", "operator")

    def _raise(**_kwargs):
        raise NetworkDiscoveryError("nmap bulunamadı")

    monkeypatch.setattr(network_api, "discover_devices", _raise)
    resp = client.post(
        "/network/discover",
        json={"target_cidr": "192.168.1.0/24"},
        headers={"X-Session-Token": token},
